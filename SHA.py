...
# 大端打包
def pack_big_endian(*values):
    packed_data = []
    for value in values:
        for _ in range(4):
            packed_data.append((value >> 24) & 0xFF)
            value <<= 8
...
def sha256(message: bytes) -> bytes:
    ...
    remainder = len(message) % 64 + 1
    if len(message) % 64 < 56:
        padded += b'\x00' * (56 - remainder)
    else:
        padded += b'\x00' * (120 - remainder)
    ...
    return pack_big_endian(h0, h1, h2, h3, h4, h5, h6, h7)
