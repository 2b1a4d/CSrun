...
class AES:
    ...
    # 字节替换，按SBox替换输入的16字节
    @staticmethod
    def sub_byte(state: list[int], inv: bool):
        # 生成密钥时长度为4 加解密时长度为16
    ...
    # 行移位
    @staticmethod
    def shift_row(state: list[int], inv: bool):
        temp: list[int] = []
        for i in range(16):
            if not inv:
    ...
    # 轮密钥加
    @staticmethod
    def add_roundkey(state: list, key: list[int]):
        for i in range(16):
            state[i] ^= key[i]
    ...
    # 填充
    @staticmethod
    def padding(text: bytes, inv: bool) -> bytes:
        if not inv:
            res = text
            num = 16 - len(text)
            for i in range(num):
